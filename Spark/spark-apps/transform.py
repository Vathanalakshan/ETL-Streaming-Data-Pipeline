...
station_status_schema=ArrayType(StructType([
...
station_information_schema=ArrayType(StructType([
...
df_status=df_status.selectExpr("CAST(key AS STRING)", "CAST(value AS STRING)") \
    .withColumn('value',from_json('value',station_status_schema))\
    .select('key',explode_outer('value').alias('value'))\
    .select("key",col('value.*'))\
    .withColumns({'is_installed': col("is_installed").cast(BooleanType()),
                  'is_renting': col("is_renting").cast(BooleanType()),
                  'is_returning': col("is_returning").cast(BooleanType())})\
    .withColumn('num_bikes_mechanic', regexp_extract(col('num_bikes_available_types'),r"\{\"mechanical\":?([,\d]+)\]?\}", 1))\
    .withColumn('num_bikes_ebike', regexp_extract(col('num_bikes_available_types'),r"\{\"ebike\":?([,\d]+)\]?\}", 1))\
    ...
    .drop("key")
df_information=df_information.selectExpr("CAST(key AS STRING)", "CAST(value AS STRING)") \
...
df_information.join(df_status,["timestamp","station_id"],'inner')\
.writeStream \
